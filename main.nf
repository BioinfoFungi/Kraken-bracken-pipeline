...
process kraken2 {
    ...
    conda '/ssd1/wy/.conda/envs/kraken2'
    ...
    output:
        tuple val(sample_id), file('*_kr.out') 
        tuple val(sample_id), path('*.report.txt') 
...
process bracken {
    conda '/ssd1/wy/.conda/envs/bracken'
...
process counts_parse {
    ...
    conda '/ssd1/wy/.conda/envs/perl-sort-key'
    ...
    """
    echo `pwd`
    perl /ssd1/wy/workspace/nextflow/Kraken-bracken-pipeline/bin/counts_parser_nxt.pl -dir "${abundances}" -label "Experiment"
    """
}
...
workflow{
    ...
    kraken2(krakendb_ch,read_ch)
    // println kraken2
    // kraken2.out[1].view{"${it}"}
    bracken( kraken2.out[1], brackendb_ch)
    println bracken.collect()
    // println bracken.out
    counts_parse(bracken.out.collect())
}
